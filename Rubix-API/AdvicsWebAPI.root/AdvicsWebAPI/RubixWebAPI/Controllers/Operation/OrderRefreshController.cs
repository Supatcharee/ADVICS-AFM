...
{
    public class OrderRefreshController : ApiController
    {
        OrderRefresh obj = new OrderRefresh();
        ...
        {
            List<sp_ROMS010_OrderRefresh_SearchAll_Receiving_Result> ResultReturn;
            try
            {
        ...
        public HttpResponseMessage SaveDataReceiving()
        {
            ...
            List<sp_ROMS010_OrderRefresh_SearchAll_Receiving_Result> data = JsonConvert.DeserializeObject<List<sp_ROMS010_OrderRefresh_SearchAll_Receiving_Result>>(strParameterData);

            try
            {
                obj.SaveDataReceiving(data);
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, Rubix.Framework.ExceptionManager.ResponseException(ex));
        ...
        {
            List<sp_ROMS010_OrderRefresh_SearchAll_Shipping_Result> ResultReturn;
            try
            {
        ...
        {
            string strParameterData = Request.Content.ReadAsStringAsync().Result;
            List<sp_ROMS010_OrderRefresh_SearchAll_Shipping_Result> data = JsonConvert.DeserializeObject<List<sp_ROMS010_OrderRefresh_SearchAll_Shipping_Result>>(strParameterData);
