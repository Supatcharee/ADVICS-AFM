...
{
    public class ExportInvoiceReport : BusinessFactory.Report.Base.BaseReport
    {
        public DataSet GetDataReport(string ShipmentNo, int Installment, string ContainerNo)
        {
            try
            {
                ...
                DataSet ds = Database.ExecuteDataSet("sp_RPT038_ExportInvoice_GetData", hash);

                if (ds.Tables.Count == 3)
                {
                    ds.DataSetName = "dsRPT038_ExportInvoiceReport";
