...
{
    public partial class dlgASO011_SaleOrder : DialogBase
    {
        #region Member
            ...
            try
            {
                if (ofdImport.ShowDialog(this) == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    using (excel)
                    {
                        ...
                        try
                        {
                            if (DateTime.ParseExact("01/" + DueDate, "dd/MMM/yy", CultureInfo.InvariantCulture) == null)
                            {
                                errorMsg += Common.GetMessage("I0279", "Due Date", DueDate) + Environment.NewLine;
                            }
                            else
                            {
                                DateTime ShippingPeriod = DateTime.ParseExact("01/" + DueDate, "dd/MMM/yy", CultureInfo.InvariantCulture);
                        ...
                        try
                        {
                            if (!BusinessClass.SaleOrderCheckValidCustomerPO(CustomerPONo, DateTime.ParseExact("01/" + DueDate, "dd/MMM/yy", CultureInfo.InvariantCulture)))
                            {
                        ...
                        else
                        {
                            ...
                            dtShippingPeriod.EditValue = DateTime.ParseExact("01/" + DueDate, "dd/MMM/yy", CultureInfo.InvariantCulture);
        ...
        private void dlgASO011_SaleOrder_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                itemPriceControl.PriceType = Control.ItemPriceControl.ePriceType.ImportSale;
        ...
        private void customerControl_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                if (customerControl.CustomerID != null)
                {
                    ...
                    itemPriceControl.DataLoading();
                    List<sp_common_LoadProductPrice_Result> list = ProductPriceBusiness.DataLoading(ownerControl.OwnerID, warehouseControl.WarehouseID, customerControl.CustomerID, Control.ItemPriceControl.ePriceType.ImportSale.ToString());
                    repProductCode.DataSource = list;
                    repProductName.DataSource = list;
        ...
        private void itemPriceControl_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                if (itemPriceControl.ProductID != null)
                {
                    ...
                    txtPurchasePrice.EditValue = itemPriceControl.UnitPrice2;
                    txtPurchasePriceCurrency.Text = itemPriceControl.CurrencyName2;
        ...
        private void SaveData()
        {
            ...
            {                
                ...
                BusinessClass.SaleOrderSaveData(this.SONo, ownerControl.OwnerID, warehouseControl.WarehouseID, customerControl.CustomerID,
                                                dtShippingPeriod.DateTime.ToString("yyyy/MM/01"),
                                                dtCustomerPOIssueDate.EditValue != null ? dtCustomerPOIssueDate.DateTime.ToString("yyyy/MM/dd") : null,
                                                txtCustomerPONo.Text.Trim(), txtValidity.Text.Trim(), txtRemark.Text.Trim(), 0, ds.GetXml());
            }
            finally
            {
                ds.Tables.Clear();
        ...
        private bool ValidateHeader()
        {
            ...
            else if (dtShippingPeriod.DateTime.Month < DateTime.Today.Month && dtShippingPeriod.DateTime.Year < DateTime.Today.Year)
            {
